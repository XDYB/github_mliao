...
AHomePageMenu1 = kd.inherit(kd.Layer);
...
function AHomePageMenu1:init()
	...
	--	注册数据中心
	DC:RegisterCallBack("AHomePageMenu1.Show",self,function(bool)
		self:SetVisible(bool)
	end);
	
	self.m_IsSwitch = true;
	DC:FillData("AHomePageMenu1.IsSwitch",self.m_IsSwitch);

	--	是否悬浮
	self.m_IsSuspend = false;
	DC:FillData("AHomePageMenu1.IsSuspend",self.m_IsSuspend);
	
	--	注册数据中心
	DC:RegisterCallBack("AHomePageMenu1.SwitchTextColor",self,function(index)
		if index ~= nil then
...
function AHomePageMenu1:SetVisible(visible)
	kd.Node.SetVisible(self, visible);
end

function AHomePageMenu1:onTouchBegan(x,y)
	echo("顶部菜单栏点击开始");
...
function AHomePageMenu1:onTouchMoved(x,y)
	echo("顶部菜单栏点击移动");
	return true;
end

function AHomePageMenu1:onTouchEnded(x,y)
	echo("顶部菜单栏点击结束");
	return true;
end

--	点击事件
function AHomePageMenu1:onGuiToucBackCall(id)
	local index = 0;
...
-- 切换文字颜色
function AHomePageMenu1:SwitchTextColor(index)
...
--	获取九宫格状态值
function AHomePageMenu1:GetIndex()
	self.m_Index = DC:GetData("AHomePageTitle.Index");
end

function AHomePageMenu1:SetData(data)

end

function AHomePageMenu1:SetIsSwitch(bvisible)
	DC:FillData("AHomePageMenu1.IsSwitch",bvisible);
end	

--	模板高度
function AHomePageMenu1:GetWH()
	return ScreenW,240;
end