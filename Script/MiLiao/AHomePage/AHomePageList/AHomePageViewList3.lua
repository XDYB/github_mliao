...
AHomePageViewList3 = kd.inherit(kd.Layer);
...
function AHomePageViewList3:init()
	...
	self.Scroll.OnScroll= function(this,deltaY,totalDeltaY,preDeltaY)
		...
		local bo = DC:GetData("AHomePageMenu3.IsSwitch");
		if totalDeltaY<-(330 + 25+x_IpY) and bo then
			DC:CallBack("AHomePageMenu3.Show",true);
			DC:FillData("AHomePageMenu3.IsSuspend",true);
		else
			if totalDeltaY>-(330 + 25+x_IpY) then
				DC:CallBack("AHomePageMenu3.Show",false);
				DC:FillData("AHomePageMenu3.IsSuspend",false);
			end
		end
	end
	...
	--	注册数据中心
	DC:RegisterCallBack("AHomePageViewList3.Show",self,function(bool)
		self:SetVisible(bool);
	end);
	
	-- 切换话题详情头部图片
	DC:RegisterCallBack("AHomePageViewList3.SetData",self,function(data)
		if data ~= nil then
			self:SetData(data);
		end
	end);
	...
	-- 实时更新拉黑数据
	DC:RegisterCallBack("AHomePageViewList3.DeleteUserID",self,function(userID)
		if userID ~= nil then
			self:DeleteUserID(userID);
		end
	end);
	
	DC:RegisterCallBack("AHomePageViewList3.InsertUser",self,function(userID)
		if userID ~= nil then
...
function AHomePageViewList3:SetVisible(visible)
	kd.Node.SetVisible(self, visible);
end

function AHomePageViewList3:onTouchBegan(x,y)
	echo("首页点击开始");
	return true;
end

function AHomePageViewList3:onTouchMoved(x,y)
	echo("首页点击移动");
	return true;
end

function AHomePageViewList3:onTouchEnded(x,y)
	echo("首页点击结束");
	return true;
end

function AHomePageViewList3:update()
	local totalDeltaY = self.Scroll:GetScrollViewH();
	local bo = DC:GetData("AHomePageMenu3.IsSwitch");
	if totalDeltaY<-(330 + 25+x_IpY) and bo then
		DC:CallBack("AHomePageMenu3.Show",true);
...
function AHomePageViewList3:onGuiToucBackCall(id)

end

--	列表数据
--	@type  用来判断网络状态，网络中断状态
function AHomePageViewList3:SetData(data)
...
function AHomePageViewList3:copy(orig)
	local copytb;
...
--	设置固定模板
function AHomePageViewList3:SetFiexdTemplate(index)
	self.m_nIndex[1] = 1;
	self.m_nIndex[2] = 2;
	self.m_datas[1] = 0;
...
--设置模板类型
function AHomePageViewList3:SetTemplate(data)
	if data == nil then return;end
...
-- 坐标偏移
function AHomePageViewList3:SetOffSet(datas,offsize)
...
-- 拉黑回调
function AHomePageViewList3:DeleteUserID(userID)
...
-- 恢复拉黑回调
function AHomePageViewList3:InsertUser(userID)
	if userID == nil then return;end
...
--黑名单匹配
function AHomePageViewList3:ClearHateList(datas)
...
-- 清理
function AHomePageViewList3:Cls()
	self.Scroll:DelAll();
	self.m_nIndex = {};
	self.m_datas = {};
	self.m_page = 0 ;				-- 当前页
