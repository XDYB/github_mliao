...
AHomePageViewList1 = kd.inherit(kd.Layer);
...
function AHomePageViewList1:init()
	...
	self.Scroll.OnGetTemplate = function(this,index)
		...
		local bo = DC:GetData("AHomePageMenu1.IsSwitch");
		if totalDeltaY<-(330 + 25+x_IpY) and bo then
			DC:CallBack("AHomePageMenu1.Show",true);
			DC:FillData("AHomePageMenu1.IsSuspend",true);
		else
			if totalDeltaY>-(330 + 25+x_IpY) then
				DC:CallBack("AHomePageMenu1.Show",false);
				DC:FillData("AHomePageMenu1.IsSuspend",false);
			end
		end
	end
	...
	--	注册数据中心
	DC:RegisterCallBack("AHomePageViewList1.Show",self,function(bool)
		self:SetVisible(bool);
	end);
	
	-- 切换话题详情头部图片
	DC:RegisterCallBack("AHomePageViewList1.SetData",self,function(data)
		if data ~= nil then
			self:SetData(data);
		end
	end);
	...
	-- 实时更新拉黑数据
	DC:RegisterCallBack("AHomePageViewList1.DeleteUserID",self,function(userID)
		if userID ~= nil then
			self:DeleteUserID(userID);
		end
	end);
	
	DC:RegisterCallBack("AHomePageViewList1.InsertUser",self,function(userID)
		if userID ~= nil then
...
function AHomePageViewList1:SetVisible(visible)
	kd.Node.SetVisible(self, visible);
end

function AHomePageViewList1:onTouchBegan(x,y)
	echo("首页点击开始");
	return true;
end

function AHomePageViewList1:onTouchMoved(x,y)
	echo("首页点击移动");
	return true;
end

function AHomePageViewList1:onTouchEnded(x,y)
	echo("首页点击结束");
	return true;
end

function AHomePageViewList1:update()
	local totalDeltaY = self.Scroll:GetScrollViewH();
	local bo = DC:GetData("AHomePageMenu1.IsSwitch");
	if totalDeltaY<-(330 + 25+x_IpY) and bo then
		DC:CallBack("AHomePageMenu1.Show",true);
...
function AHomePageViewList1:onGuiToucBackCall(id)

end

--	列表数据
--	@type  用来判断网络状态，网络中断状态
function AHomePageViewList1:SetData(data)
...
function AHomePageViewList1:copy(orig)
	local copytb;
...
--	设置固定模板
function AHomePageViewList1:SetFiexdTemplate(index)
	self.m_nIndex[1] = 1;
	self.m_nIndex[2] = 2;
	self.m_datas[1] = 0;
...
--设置模板类型
function AHomePageViewList1:SetTemplate(data)
	if data == nil then return;end
...
-- 坐标偏移
function AHomePageViewList1:SetOffSet(datas,offsize)
...
-- 拉黑回调
function AHomePageViewList1:DeleteUserID(userID)
...
-- 恢复拉黑回调
function AHomePageViewList1:InsertUser(userID)
	if userID == nil then return;end
...
--黑名单匹配
function AHomePageViewList1:ClearHateList(datas)
...
-- 清理
function AHomePageViewList1:Cls()
	self.Scroll:DelAll();
	self.m_nIndex = {};
	self.m_datas = {};
	self.m_page = 0 ;				-- 当前页
