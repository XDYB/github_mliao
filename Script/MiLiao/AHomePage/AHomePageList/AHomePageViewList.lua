...
AHomePageViewList = kd.inherit(kd.Layer);
...
function AHomePageViewList:init()
	...
	self.Scroll.OnScroll= function(this,deltaY,totalDeltaY,preDeltaY)
		...
		local bo = DC:GetData("AHomePageMenu2.IsSwitch");
		echo(bo);
		if totalDeltaY<-(330 + 25+x_IpY) and bo then
			DC:CallBack("AHomePageMenu2.Show",true);
			DC:FillData("AHomePageMenu2.IsSuspend",true);
		else
			if totalDeltaY>-(330 + 25+x_IpY) then
				DC:CallBack("AHomePageMenu2.Show",false);
				DC:FillData("AHomePageMenu2.IsSuspend",false);
			end
		end
	end
	...
	--	注册数据中心
	DC:RegisterCallBack("AHomePageViewList.Show",self,function(bool)
		self:SetVisible(bool);
	end);
	
	-- 切换话题详情头部图片
	DC:RegisterCallBack("AHomePageViewList.SetData",self,function(data)
		if data ~= nil then
			self:SetData(data);
		end
	end);
	self.m_tempData = {};
	DC:FillData("AHomePageViewList.data",self.m_tempData);
	
	-- 实时更新拉黑数据
	DC:RegisterCallBack("AHomePageViewList.DeleteUserID",self,function(userID)
		if userID ~= nil then
			self:DeleteUserID(userID);
		end
	end);
	
	DC:RegisterCallBack("AHomePageViewList.InsertUser",self,function(userID)
		if userID ~= nil then
...
function AHomePageViewList:SetVisible(visible)
	kd.Node.SetVisible(self, visible);
end

function AHomePageViewList:onTouchBegan(x,y)
	echo("首页点击开始");
	return true;
end

function AHomePageViewList:onTouchMoved(x,y)
	echo("首页点击移动");
	return true;
end

function AHomePageViewList:onTouchEnded(x,y)
	echo("首页点击结束");
	return true;
end

function AHomePageViewList:update()
	local totalDeltaY = self.Scroll:GetScrollViewH();
	local bo = DC:GetData("AHomePageMenu2.IsSwitch");
	if totalDeltaY<-(330 + 25+x_IpY) and bo then
		DC:CallBack("AHomePageMenu2.Show",true);
...
function AHomePageViewList:onGuiToucBackCall(id)

end

--	列表数据
--	@type  用来判断网络状态，网络中断状态
function AHomePageViewList:SetData(data)
	...
	self:SetOffSet(mdata,nil);
	self.Scroll:SetData(mdata);
	if self.m_IsInsert then
		DC:CallBack("AHomePageMenu2.Show",false);
		DC:CallBack("AHomePageMenu2.SwitchTextColor",index);
	end
end

function AHomePageViewList:copy(orig)
	local copytb;
...
--	设置固定模板
function AHomePageViewList:SetFiexdTemplate(index)
	self.m_nIndex[1] = 1;
	self.m_nIndex[2] = 2;
	self.m_datas[1] = 0;
...
--设置模板类型
function AHomePageViewList:SetTemplate(data)
	if data == nil then return;end
...
-- 坐标偏移
function AHomePageViewList:SetOffSet(datas,offsize)
...
-- 拉黑回调
function AHomePageViewList:DeleteUserID(userID)
...
-- 恢复拉黑回调
function AHomePageViewList:InsertUser(userID)
	if userID == nil then return;end
	...
	for i = #self.m_DelData,1,-1 do
		if userID == self.m_DelData[i].UserId then
			...
			table.insert(self.m_datas,self.DelData[i]);
...
--黑名单匹配
function AHomePageViewList:ClearHateList(datas)
...
-- 清理
function AHomePageViewList:Cls()
	self.Scroll:DelAll();
	self.m_nIndex = {};
	self.m_datas = {};
	self.m_page = 0 ;				-- 当前页
