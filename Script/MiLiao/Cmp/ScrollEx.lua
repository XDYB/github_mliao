...
ScrollEx = kd.class(kd.Layer);
function ScrollEx:SetMarginTop(x)
	self.marginTop = x
end
function ScrollEx:init(x,y,w,h,param)
	self.x = x
	self.y = y
	self.w = w
	self.h = h
...
--	self.guilayer:setDebugMode(true)
	...
	self.momentumTimeThreshold = 120
...
-- 获取节点
function ScrollEx:OnDraw(index)
...
function ScrollEx:onTouchBegan(x,y)
...
function ScrollEx:update(delta)
	if self.sprLoadRot then
...
--	if #self.datas>0 or self.exNodeHeight>0 then
		...
		for i=index-1,index+self.OptimizeCount-1 do
			if self.nodemap:containsKey(i)==false then
...
function ScrollEx.clock()
	return os.clock() * 1000
end
function ScrollEx:onTouchMoved(x,y)
...
function ScrollEx:momentum(current, start, duration)
	local durationMap = {
		noBounce=2500,
		weekBounce=800,
		strongBounce=400,
...
function ScrollEx:onTouchEnded(x,y)
...
-- @index 	索引
-- @item 	节点
function ScrollEx:SetItemPos(index,node)
	if node then
		local posHeight = 0
		local realHeight = 0
		local tempWidth = 0
...
-- 获取模版
function ScrollEx:GetTemplate(index)
...
-- 左滑检测
function ScrollEx:IsLeftToRight(x,y)
...
-- 检测右滑
function ScrollEx:IsRightToLeft(x,y)
...
-- @y 滚动y值
-- @isAni 是否动画
function ScrollEx:SetOffSet(y,isAni)
	if isAni then
		TweenPro:Animate({
...
-- @index 下标
-- @data 单个数据
function ScrollEx:UpdateData(index,data)
...
-- 追加数据到末尾
-- @data 单个数据
function ScrollEx:AppendData(data)
	local len = #self.datas
	self.datas[len+1] = data
end
-- 设置数据
-- @datas 数据数组
function ScrollEx:SetData(datas,extData)
	self:DelAll()
...
-- 获取数据
function ScrollEx:GetData()
	return self.datas
end
-- 删除数据
function ScrollEx:DelByIndex(index)
	if #self.datas>=index then
		...
		echo("=======ScrollEx:DelByIndex 越界")
	end
end
-- 删除全部
function ScrollEx:DelAll()
	for i,v in each(self.nodemap) do
		if v.Cls then 
			v:Cls();
		end
...
function ScrollEx:ClearSpeTemplate()
...
function ScrollEx:ChangeTemplate(index,TempateClazz)
...
-- =====================================================================
-- 设置显示的节点数
function ScrollEx:SetOptimizeCount(i)
	self.OptimizeCount = i
end

-- 打开加载动画
function ScrollEx:OpenAni()
	self.hasLoadAni = true
end

-- 显示上拉等待
function ScrollEx:ShowUpLoad(spr)
	self.sprLoadRot = 1
	self.layer:addChild(spr)
	self.sprLoad=spr
end
-- 隐藏上拉等待
function ScrollEx:HideUpLoad()
...
function ScrollEx:AddNode(node)
	local w,h = node:GetWH()
	self.layer:addChild(node)
...
--	用于窗口浮动
function ScrollEx:GetScrollViewH()
	local x,y = self.layer:GetPos();
	local scrolly = y - ScreenH;
	return scrolly;
end
...
-- =====================================================================
-- 下拉加载等待
function ScrollEx:AddDownLoad(clazz)
...
-- 上拉加载等待
function ScrollEx:AddUpLoad(clazz)
...
-- 设置上拉加载位置
function ScrollEx:SetUpLoadPos()
	if self.upload then
		local _,h = self.upload:GetWH()
		if self.realHeight<self.h then
			self.upload:SetPos(0,ScreenH+self.y+h/2+self.h)
...
-- 设置底部文字显示
function ScrollEx:SetTxtBottomPos()
	self.bottomTextH = 150
	self.txtBottom:SetPos(ScreenW/2,self.y+self.realHeight-self.bottomTextH/2)
end

function ScrollEx:NoMore()
	self.upload:NoMore()
end

-- 显示底部文字
function ScrollEx:ShowBottomText(str)
