...
function impl:onTouchMoved(--[[float]] x, --[[float]] y)
	...
	if offsetY > self.maxY then
		...
		if self.download==nil then	--没有下拉刷新就不用移动视图
			return;
		end
...
-- 复位到底部
function impl:BackBottomNoAni()
	local x,_ = self.layer:GetPos()
	self.layer:SetPos(x,self.minY)
end
-- 复位到顶部
function impl:BackTopNoAni()
	local x,_ = self.layer:GetPos()
	self.layer:SetPos(x,self.maxY)
end
