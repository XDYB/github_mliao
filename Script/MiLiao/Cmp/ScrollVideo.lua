...
ScrollVideo = kd.inherit(kd.Layer);
local ScrollView = ScrollVideo;
...
function ScrollView:SetViewPos(x,y,bStop)
	...
	--if(self.m_bDirectX==false) then kd.LogOut("KD_LOG:ScrollView-----SetViewPos x="..x..",y="..y); end
end
...
--复位完成回调(需要自己继承实现)
function ScrollView:OnAppointBackEnd(x,y)
...
function ScrollView:UpdateBackSpeed(posx,posy)
	...
	if(math.abs(self.m_fBackSpeed)<=0.01) then 
		...
		elseif(self.m_bApBack) then
			...
			self:SetViewPos(x,y);
			if self.OnAppointBackEnd then
				self:OnAppointBackEnd();
			end
