...
ScrollExA = kd.class(kd.Layer);
local impl = ScrollExA
function impl:SetMarginTop(x)
	self.marginTop = x
end
...
function impl:onTouchBegan(x,y)
...
function impl.clock()
--	local t = kd.GetLocalTime()
--	local ms = t.Hour*3600*1000 + t.Minute*60*1000+t.Second*1000+t.Milliseconds
	return os.clock() * 1000
--	return ms
end
function impl:onTouchMoved(x,y)
...
function impl:onTouchEnded(x,y)
	...
	-- ======================================================
		...
		-- Test START
--		A_ViewManager:messagebox_default(duration.."/"..absDeltaY)
		-- Test E
		-- ====================有惯性=====================
					...
					if self.OnClick then
						...
						for i,v in ipairs(self.datas) do
							...
							if node then
								...
								local nw,nh = node:GetWH()
								local x1 = nx
								local y1 = ny - (ScreenH+self.y)
								local x2 = nx+nw
								local y2 = ny+nh - (ScreenH+self.y)
								if fx>=x1 and fx<=x2 and fy>=y1 and fy<=y2 then
									index = i
									break
								end
							end
...
-- @index 	索引
-- @item 	节点
function impl:SetItemPos(index,node)
	if node then
		local leftHeight = 0 -- 左高
		local rightHeight = 0-- 右高
		local realHeight = 0
		local tempWidth = 0
		local x = 0
		local y = 0
		for i=1,index do
			...
			if w<ScreenW/2 then
				if leftHeight<=rightHeight then
					-- 左
					x = 0
					y = ScreenH + self.y + leftHeight
					leftHeight = leftHeight + h
				else
					-- 右
					x = w
					y = ScreenH + self.y + rightHeight
...
-- 获取模版
function impl:GetTemplate(index)
...
-- 左滑检测
function impl:IsLeftToRight(x,y)
...
-- 检测右滑
function impl:IsRightToLeft(x,y)
...
function impl:BackTop()
	TweenPro:Animate({
		{o=self.layer,y=self.maxY,speed_bezier={.25, .46, .45, .94},fn=function()
			if self.download then
				self.download:BackTop()
			end
		end}
	});
end
...
function impl:ToIndex(i)
	...
	if i==1 then
		...
		local finalPos = ScreenH-posHeight+tmpH
		if finalPos<self.minY then
			finalPos = self.minY
		end
		self.layer:SetPos(0,finalPos)
	end
end
-- 设置偏移量
-- @y 滚动y值
-- @isAni 是否动画
function impl:SetOffSet(y,isAni)
	if isAni then
		TweenPro:Animate({
...
-- @index 下标
-- @data 单个数据
function impl:UpdateData(index,data)
...
-- 追加数据到末尾
-- @data 单个数据
function impl:AppendData(data)
	local len = #self.datas
	self.datas[len+1] = data
end
-- 设置数据
-- @datas 数据数组
function impl:SetData(datas,extData)
...
function impl:ClearSpeTemplate()
...
function impl:ChangeTemplate(index,TempateClazz)
...
-- =====================================================================
-- 设置显示的节点数
function impl:SetOptimizeCount(i)
	self.OptimizeCount = i
end

-- 打开加载动画
function impl:OpenAni()
...
function impl:equal(value1, value2)
