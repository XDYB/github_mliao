...
AScroll = kd.class(kd.Layer);

function AScroll:init(x,y,w,h)
	self.x = x
	self.y = y
	self.w = w
	self.h = h
...
function AScroll:SetMinMax(min,max)
	self.min = min
	self.max = max
end
function AScroll:CalMiny()
	local min = 0
...
function AScroll:onTouchBegan(--[[float]] x, --[[float]] y)
	...
	if y>self.y+self.min and y<self.y+self.max then
		self.isSendExScrollEvent = true
		DC:CallBack("AScroll.onTouchBegan",x,y)
	end
end


	
function AScroll.clock()
	return os.clock() * 1000 -- 秒
end
function AScroll:onTouchMoved(--[[float]] x, --[[float]] y)
	...
	elseif  self.lock == 1 then
		if self.isSendExScrollEvent then
			self.m_isCanChangeLock = true
			DC:CallBack("AScroll.onTouchMoved",x,y)
		end
	end
...
function AScroll:onTouchEnded(--[[float]] x, --[[float]] y)
	...
	elseif self.lock == 1 and self.isSendExScrollEvent then
		...
		-- 水平滚动
		-- ==============================
		DC:CallBack("AScroll.onTouchEnded",x,y)
		self.isSendExScrollEvent = false
	end
	self.m_isCanChangeLock = true
...
function AScroll:momentum(current, start, duration)
	local durationMap = {
		noBounce=2500,
		weekBounce=800,
		strongBounce=400,
...
-- 添加项(动画版)
-- @enumAddAni 枚举 出场动画
function AScroll:Add_Ani(item,enumAddAni)
	local enum = enumAddAni or AScroll.enumAddAni.bottom
	if self.isOptimize then
	...
	elseif enum == AScroll.enumAddAni.rotz then
		item:SetPos(0,ScreenH+self.posHeight+self.y);
	...
	elseif enum == AScroll.enumAddAni.scale2 then
		item:SetPos(0,ScreenH+self.posHeight+self.y);
		item:SetScale(1.3,1.3)
		TweenPro:Animate({
			{o=item,scale = 1,x=x,y=y,tween = TweenPro.swing.easeOutExpo,d=800}
		});
	elseif enum == AScroll.enumAddAni.bsr1 then
	...
	elseif enum == AScroll.enumAddAni.bsr2 then
		item:SetPos(ScreenW,ScreenH+self.posHeight+self.y+ScreenH/2);
		TweenPro:Animate({
			{o=item,x=x,y=y,tween = TweenPro.swing.easeOutExpo,d=800,
	...
	-- 复杂的自定义
	elseif enum == AScroll.enumAddAni.cus then
		local isLeft = true
...
-- @index 下标
function AScroll:DelByIndex_Ani(index)
...
-- 删除项
-- @index 下标
function AScroll:DelByIndex(index)
	local item = table.remove(self.nodes,index)
	local w,h= item:GetWH()
...
function AScroll:OpenOptimize()
	self.isOptimize =  true
end
-- (Test)关闭滚动优化 总是将所有的节点设置为可见状态 元素过多会降低帧率
function AScroll:CloseOptimize()
	self.isOptimize =  false
end
-- 优化数量 一屏范围内显示的子元素个数 一屏元素过多时 请调用该方法手动调节 默认15
function AScroll:SetOptimizeCount(c)
	self.OptimizeCount = c
end

-- 根据下标获取节点
function AScroll:GetItemByIndex(i)
	return self.nodes[i]
end
-- 获取所有节点
function AScroll:GetItems()
	return self.nodes
end
-- 根据ID查询节点
function AScroll:GetItemByID(id)
	local item
	for i,v in ipairs(self.nodes) do
		if v.id == id then
			item = v
...
-- =======================================================================
--[[
self.AScroll = kd.class(AScroll,true,true)
self.AScroll:init(0,50,ScreenW,ScreenH-50)
self:addChild(self.AScroll)
self.AScroll.OnClick = function(this,index,item)
	echo("==========点击"..index)
end

self.AScroll.OnDown = function(this)
	echo("==========下拉")
	this:BackPos() -- 复位
end
self.AScroll.OnUp = function(this)
	echo("==========上拉")
	this:BackPos() -- 复位
end
for i=1,100 do
