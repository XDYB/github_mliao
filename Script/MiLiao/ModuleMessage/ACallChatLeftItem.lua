...
ACallChatLeftItem = kd.inherit(kd.Layer);
local impl = ACallChatLeftItem
local ids =
{
	--/* Image ID */
...
function impl:init(data,bYuYin)
	...
	self.m_thView = kd.class(kd.ResManage, gDef.GetResPath("ResAll/UI/XXQiPao1.UI"), self);
	self:addChild(self.m_thView);
	
	local x,y = self.m_thView:GetPos()
	self.m_thView:SetPos(x + 150,y)
	
	self.sprTopL = self.m_thView:GetSprite(ids.ID_IMG_ML_MAIN2_LM)
	local topL_x,topL_y = self.sprTopL:GetPos()
	local topL_w,topL_h = self.sprTopL:GetWH()
	self.sprTopC = self.m_thView:GetSprite(ids.ID_IMG_ML_MAIN_LM0)
	local topC_x,topC_y = self.sprTopC:GetPos()
	local topC_w,topC_h = self.sprTopC:GetWH()
	self.sprTopR = self.m_thView:GetSprite(ids.ID_IMG_ML_MAIN_LM1)
	local topR_w,_ = self.sprTopR:GetWH()
	
	self.sprCenter = self.m_thView:GetSprite(ids.ID_IMG_ML_MAIN_LM3)
	self.sprBtmL = self.m_thView:GetSprite(ids.ID_IMG_ML_MAIN_LM5)
	local btmL_x,_ = self.sprBtmL:GetPos()
	local btmL_w,btmL_h = self.sprBtmL:GetWH()
	self.sprBtmC = self.m_thView:GetSprite(ids.ID_IMG_ML_MAIN_LM6)
	self.sprBtmR = self.m_thView:GetSprite(ids.ID_IMG_ML_MAIN_LM7)
	...
	if bYuYin then
		...
		self.m_sprYuYin = kd.class(kd.Sprite,gDef.GetResPath("ResAll/Main.png"),1596,438,40,40);
		
		if self.m_aniHuo then 
			for i= 1,3 do
				local spr = kd.class(kd.Sprite,gDef.GetResPath("ResAll/Main.png"),1512+(i-1)*40,438,40,40);
		...
		if self.m_sprYuYin then
			...
			self.m_sprYuYin:SetPos(WIDTH/2+100 + 130,topC_y+topC_h/2+FONT_SIZE*line/2)
		end
		
		
		if 	self.m_aniHuo then
			--self.m_aniHuo:SetMode(kd.AniPlayMode.HGEANIM_LOOP);
			self.m_aniHuo:SetPos(WIDTH/2+100 + 130,topC_y+topC_h/2+FONT_SIZE*line/2)
		end
	end
	...
	self.txt:SetPos(WIDTH/2+100 + 150,topC_y+topC_h/2+FONT_SIZE*line/2);
	self.txt:SetColor(0xffffffff)
	
	-- 上
	self.sprTopC:SetScale(txtW/10,1)
	local _,y = self.sprTopC:GetPos()
	self.sprTopC:SetPos(topL_x+topL_w/2+txtW/2,y)
	self.sprTopR:SetPos(topL_x+topL_w/2+txtW+topR_w/2,y)
	-- 中
	self.sprCenter:SetScale((txtW+topL_w+topL_w)/10,(FONT_SIZE*line)/10)
	self.sprCenter:SetPos(topL_x-topL_w/2+(txtW+topL_w+topL_w)/2,topL_y+topL_h/2+FONT_SIZE*line/2)
	-- 下
	self.sprBtmL:SetPos(btmL_x,topL_y+topL_h/2+(FONT_SIZE*line)+btmL_h/2)
	self.sprBtmC:SetScale(txtW/10,1)
	self.sprBtmC:SetPos(topL_x+topL_w/2+txtW/2,topL_y+topL_h/2+(FONT_SIZE*line)+btmL_h/2)
	self.sprBtmR:SetPos(topL_x+topL_w/2+txtW+topR_w/2,topL_y+topL_h/2+(FONT_SIZE*line)+btmL_h/2)
	...
	local tmp = gDef:gradientColor("0FFCA4C","0FF76DA1",50);--#F76DA1
	local color = "0xff"..tmp;
	self:SetColor(0xffF76DA1);
	
	--头像
	self.m_thView = kd.class(kd.ResManage, gDef.GetResPath("ResAll/UI/XXDuiHuaTouXiangZuo.UI"), self);
	self:addChild(self.m_thView);
	local avatar  = DC:GetData("AChat.right_avater",avatar)
	self.m_cusRightFace = gDef:SetFaceForGame(self,ids.ID_CUS_ML_TX147_LM,avatar,false,2);
end
