...
FocusListView = kd.inherit(kd.Layer);
local impl = FocusListView;
...
--	self.Scroll:ShowBorder()
	...
	DC:RegisterCallBack("FocusListView.DelBlackItem",self,function(id)
...
function impl:SendData(page)
	self:SetPage(page);
	gSink:Post("michat/get-lovelist",{page = page},function(data)
		if data.Result then
			self:SetData(data)
		end
	end)
