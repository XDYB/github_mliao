...
AMeDynamicTopicDetaiList = kd.inherit(kd.Layer);
...
function AMeDynamicTopicDetaiList:init()
	...
	--	注册数据中心
	DC:RegisterCallBack("AMeDynamicTopicDetaiList.Show",self,function(bool)
		self:SetVisible(bool);
	end);
	
	-- 注册数据中心(我的动态数据)
	DC:RegisterCallBack("AMeDynamicTopicDetaiList.OnRequestDynamiclist",self,function(index)
		self:OnRequestDynamiclist(index);
	end);
		
	-- 注册数据中心(Clear)
	DC:RegisterCallBack("AMeDynamicTopicDetaiList.Cls",self,function()
		self:Cls();
	end);
	
	-- 注册数据中心
	DC:RegisterCallBack("AMeDynamicTopicDetaiList.RemoveTable",self,function(index)
		if index ~= nil then 
	...
	-- 切换话题详情头部图片
	DC:RegisterCallBack("AMeDynamicTopicDetaiList.SetData",self,function(data)
		if data ~= nil then
			self:SetData(data);
		end
	end);
...
function AMeDynamicTopicDetaiList:SetVisible(visible)
	kd.Node.SetVisible(self, visible);
end

function AMeDynamicTopicDetaiList:onTouchBegan(x,y)
	echo("我的动态信息点击开始");
	return true;
end

function AMeDynamicTopicDetaiList:onTouchMoved(x,y)
	echo("我的动态信息点击移动");
	return true;
end


function AMeDynamicTopicDetaiList:onTouchEnded(x,y)
	echo("我的动态信息点击结束");
	return true;
end

function AMeDynamicTopicDetaiList:onGuiToucBackCall(id)

end

--获取动态列表数据(我的动态请求数据)
function AMeDynamicTopicDetaiList:OnRequestDynamiclist(index)
	-- 发包
	gSink:Post("michat/get-mydynamiclist",{page = 0,audit = index},function(data)
		if data.Result then
			DC:CallBack("AMeDynamicTopicDetaiList.SetData",data);
		else
...
--	列表数据
function AMeDynamicTopicDetaiList:SetData(data)
	if data == nil then return;end
	DC:CallBack("AMeDynamicTopicDetaiList.Show",true);
...
function AMeDynamicTopicDetaiList:copy(orig)
	local copytb;
...
function AMeDynamicTopicDetaiList:RemoveTable(index)
...
--设置模板类型
function AMeDynamicTopicDetaiList:SetTemplate(data)
	if data == nil then return;end
...
function AMeDynamicTopicDetaiList:Cls()
	self.Scroll:BackTop();	
	self.Scroll:DelAll();
	self.m_data = {};
	self.m_nIndex = {};
