...
AMeDynamicTopicDetaiPhoto = kd.inherit(kd.Layer);
...
function AMeDynamicTopicDetaiPhoto:init()
...
function AMeDynamicTopicDetaiPhoto:SetVisible(visible)
	kd.Node.SetVisible(self, visible);
end

function AMeDynamicTopicDetaiPhoto:Cls()
	--	大图
...
function AMeDynamicTopicDetaiPhoto:onGuiToucBackCall(id)
	-- 删除按钮
...
--	数据分发
function AMeDynamicTopicDetaiPhoto:SetData(data)
...
--	设置头像
function AMeDynamicTopicDetaiPhoto:SetUserFace(face)
	if self.avatar then
		self.avatar:Del()
		self.avatar = nil
	end
...
--	用户昵称
function AMeDynamicTopicDetaiPhoto:SetName(name)
...
--	性别图标
function AMeDynamicTopicDetaiPhoto:SetSex(index)
...
--	签名
function AMeDynamicTopicDetaiPhoto:SetSign(sign)
	if sign ==  nil then return;end
	self.m_sign:SetString(sign);
end

--	发布日期
function AMeDynamicTopicDetaiPhoto:SetTime(time)
...
--	红心值
function AMeDynamicTopicDetaiPhoto:SetHeardNum(num)
...
--	评论值
function AMeDynamicTopicDetaiPhoto:SetCommentsNum(num)
	if num == nil then return;end
	self.m_comments:SetString(num);
end

--	设置照片
function AMeDynamicTopicDetaiPhoto:SetPhoto(data)
	if data == nil or #data<=0 then return;end
	--	先隐藏
	for i = 1,4 do
		self.m_BgImg[i]:SetVisible(false);
		self.m_BgImg[i]:SetScale(1,1);
	end
	--	隐藏2张图
	for i = 1,2 do
		self.m_BgImgTwo[i]:SetVisible(false);
...
--	切换话题标签
function AMeDynamicTopicDetaiPhoto:SetChangeImg(str)
	if str == "#萌新来袭#" then						--	萌新
...
-- 我是否关注
function AMeDynamicTopicDetaiPhoto:IsFocus(index)
--	if index == nil then return;end
	self.m_redHeardImg:SetVisible(index == 1);
end

-- 是否是我的发布
function AMeDynamicTopicDetaiPhoto:IsMyRelease(bool)
...
-- @w 	头像宽度
-- @url 图片路径 
function AMeDynamicTopicDetaiPhoto:SetAvatar(thview,id,w,h,url)
...
function AMeDynamicTopicDetaiPhoto:SetBigAvatar(thview,id,w,h,url)
	local px,py,pw,ph = thview:GetScaleRect(id);
...
--	模板高度
function AMeDynamicTopicDetaiPhoto:GetWH()
	return ScreenW,1024;
end
