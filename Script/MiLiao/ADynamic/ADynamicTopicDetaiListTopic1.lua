...
ADynamicTopicDetaiListTopic1 = kd.inherit(kd.Layer);
...
function ADynamicTopicDetaiListTopic1:init()
	--==========================================================		滑动层		========================================================
	...
	self.Scroll.OnDown = function(this)
		...
		self.m_topic = str;
		DC:CallBack("ADynamicTopicDetaiListTopic1.OnRequestTopicList",str);
	end
	...
	--	注册数据中心
	DC:RegisterCallBack("ADynamicTopicDetaiListTopic1.Show",self,function(bool)
		self:SetVisible(bool);
	end);
	
	
	-- 注册数据中心(话题数据)
	DC:RegisterCallBack("ADynamicTopicDetaiListTopic1.OnRequestTopicList",self,function(str)
		if str ~= nil then
	...
	-- 切换话题详情头部图片
	DC:RegisterCallBack("ADynamicTopicDetaiListTopic1.SetData",self,function(data)
		if data ~= nil then
			self:SetData(data);
		end
	end);
	...
	-- 实时更新点赞
	DC:RegisterCallBack("ADynamicTopicDetaiListTopic1.updateIndex",self,function(data)
		if data ~= nil then
	...
	-- 实时更新拉黑数据
	DC:RegisterCallBack("ADynamicTopicDetaiListTopic1.DeleteUserID",self,function(userID)
		if userID ~= nil then
			self:DeleteUserID(userID);
		end
	end);
	
	DC:RegisterCallBack("ADynamicTopicDetaiListTopic1.InsertUser",self,function(userID)
		if userID ~= nil then
...
function ADynamicTopicDetaiListTopic1:SetVisible(visible)
	kd.Node.SetVisible(self, visible);
end

-- 结束动画后执行页面清理
function ADynamicTopicDetaiListTopic1:OnActionEnd()
	if self:IsVisible() then 
		local index = DC:GetData("ADynamicTopic.StateStr");
		DC:CallBack("ADynamicTopicDetaiHeader.SetChangeImg",index);				--	设置话题图片
		DC:CallBack("ADynamicTopicDetaiListTopic1.OnRequestTopicList",index);	--	请求话题滑动层数据
	end
end

function ADynamicTopicDetaiListTopic1:onTouchBegan(x,y)
	echo("动态话题信息点击开始");
	return true;
end

function ADynamicTopicDetaiListTopic1:onTouchMoved(x,y)
	echo("动态话题信息点击移动");
	return true;
end


function ADynamicTopicDetaiListTopic1:onTouchEnded(x,y)
	echo("动态话题信息点击结束");
	return true;
end


function ADynamicTopicDetaiListTopic1:onGuiToucBackCall(id)

end

--	请求话题列表
function ADynamicTopicDetaiListTopic1:OnRequestTopicList(str)
	self.m_topic = str;
	-- 发包
	gSink:Post("michat/get-dynamiclist",{page = 0,topic = str},function(data)
		if data.Result then
			DC:CallBack("ADynamicTopicDetaiListTopic1.SetData",data);
		else
		end
	end,false);
end
...
--	列表数据
function ADynamicTopicDetaiListTopic1:SetData(data)
...
function ADynamicTopicDetaiListTopic1:copy(orig)
	local copytb;
...
--设置模板类型
function ADynamicTopicDetaiListTopic1:SetTemplate(data)
	if data == nil then return;end
	self.m_nIndex = {};
...
--	实时更新点赞数据
function ADynamicTopicDetaiListTopic1:updateIndex(index)
...
-- 拉黑回调
function ADynamicTopicDetaiListTopic1:DeleteUserID(userID)
...
-- 恢复拉黑回调
function ADynamicTopicDetaiListTopic1:InsertUser(userID)
	if userID == nil then return;end
...
--黑名单匹配
function ADynamicTopicDetaiListTopic1:ClearHateList(datas)
...
-- 清理
function ADynamicTopicDetaiListTopic1:Cls()
	self.Scroll:ToTop();	
	self.Scroll:DelAll();
	self.m_data = {};
	self.m_nIndex = {};
