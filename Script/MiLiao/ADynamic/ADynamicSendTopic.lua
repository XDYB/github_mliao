...
ADynamicSendTopic = kd.inherit(kd.Layer);
...
function ADynamicSendTopic:init()
	...
	-- 注册数据中心
	DC:RegisterCallBack("Topic.Show",self,function(bool)
		self:Clear();
...
function ADynamicSendTopic:SetVisible(visible)
	kd.Node.SetVisible(self, visible);
end

-- 清理
function ADynamicSendTopic:Clear()
...
-- 重置所有的话题标签
function ADynamicSendTopic:ResetMessage()
	-- 删除话题,恢复原位
...
-- 初始状态
function ADynamicSendTopic:InitialState()
	self.m_btnSend:SetEnable(false);
	flag2 = nil ; 
	flag3 = nil; 
	flag4 = nil;
...
-- 检测是否可以点击上传
function ADynamicSendTopic:Check()
	self.m_text = self.m_editSign:GetText();
...
function ADynamicSendTopic:OnTextChanged()
	self.m_text = self.m_editSign:GetText();
...
function ADynamicSendTopic:onGuiToucBackCall(--[[int]] id)
...
-- 话题2,3的默认位置
function ADynamicSendTopic:TopicTwo()
	if gDef.IphoneXView then
		x_Ip = 40;
...
-- 删除话题
function ADynamicSendTopic:deleteTopic()
	self.m_sprCloseTopic:SetVisible(false);
	self.guiClose :SetVisible(false);
	self.m_txtMsg1:SetVisible(true);
	self.m_txtMsg2:SetVisible(true);
...
-- 设置话题1的颜色为灰色	
function ADynamicSendTopic:SetTopic1ColorGray()
...
-- 设置话题2的颜色为灰色	
function ADynamicSendTopic:SetTopic2ColorGray()
...
-- 设置话题3的颜色为灰色	
function ADynamicSendTopic:SetTopic3ColorGray()
...
-- 设置话题1的颜色为紫色	
function ADynamicSendTopic:SetTopic1color()
...
-- 设置话题2的颜色为紫色	
function ADynamicSendTopic:SetTopic2color()
...
-- 设置话题3的颜色为紫色	
function ADynamicSendTopic:SetTopic3color()
...
-- 上传视频或者照片
function ADynamicSendTopic:sendAll()
...
-- 上传照片
function ADynamicSendTopic:sendPhoto()
	gSink:RequestPhotoPermissions();
	local localtime = kd.GetLocalTime();
	self.m_szFileName = string.format("%u%04d%02d%02d%02d%02d%02d",		--生成文件名. 自己去拼接.mp4 或者封面的 .jpg
						147852,localtime.Year,localtime.Month, 
...
function ADynamicSendTopic:updatePhoto()
...
-- 相册回调
function ADynamicSendTopic:OnSystemPhotoRet(--[[string]] _filePath, --[[int]] fileType)
...
-- 先提交照片再提交信息
function ADynamicSendTopic:SubmitPhotos()
	self.m_photoids = {};
	self.m_tempPhoto = {}
...
-- 提交视频
function ADynamicSendTopic:SubmitVideo()
...
-- 上传图片
function ADynamicSendTopic:upload(filename,index)
	echo("上传我的图片");
...
-- 提交话题
function ADynamicSendTopic:SubmitTpoic()
	local text = nil;
	if flagTopic1 then
	...
	gSink:Post("michat/post-dynamic",Data,function(data)
		if data.Result then
			...
			gSink:ShowButtom(true);
			DC:CallBack("Topic.Show",false);
			local index = DC:GetData("ADynamicTitle.Index");
			DC:CallBack("ADynamicTitle.SetTextScale",index);			--	请求我的动态数据
		else
...
-- 获取视频信息回调
function ADynamicSendTopic:OnVideoGetInfoRetEvent(--[[int]] err_code,                  --错误码(0为成功) 
