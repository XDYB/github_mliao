...
ADynamicTopicDetaiListTopic2 = kd.inherit(kd.Layer);
...
function ADynamicTopicDetaiListTopic2:init()
	...
	self.Scroll.OnDown = function(this)
		...
		self.m_topic = str;
		DC:CallBack("ADynamicTopicDetaiListTopic2.OnRequestTopicList",str);
	end
	...
	--	注册数据中心
	DC:RegisterCallBack("ADynamicTopicDetaiListTopic2.Show",self,function(bool)
		self:SetVisible(bool);
	end);

	-- 注册数据中心(话题数据)
	DC:RegisterCallBack("ADynamicTopicDetaiListTopic2.OnRequestTopicList",self,function(str)
		if str ~= nil then
	...
	-- 切换话题详情头部图片
	DC:RegisterCallBack("ADynamicTopicDetaiListTopic2.SetData",self,function(data)
		if data ~= nil then
			self:SetData(data);
		end
	end);
	...
	-- 实时更新点赞
	DC:RegisterCallBack("ADynamicTopicDetaiListTopic2.updateIndex",self,function(data)
		if data ~= nil then
	...
	-- 实时更新拉黑数据
	DC:RegisterCallBack("ADynamicTopicDetaiListTopic2.DeleteUserID",self,function(userID)
		if userID ~= nil then
			self:DeleteUserID(userID);
		end
	end);
	
	DC:RegisterCallBack("ADynamicTopicDetaiListTopic2.InsertUser",self,function(userID)
		if userID ~= nil then
...
function ADynamicTopicDetaiListTopic2:SetVisible(visible)
	kd.Node.SetVisible(self, visible);
end

-- 结束动画后执行页面清理
function ADynamicTopicDetaiListTopic2:OnActionEnd()
	if self:IsVisible() then 
		local index = DC:GetData("ADynamicTopic.StateStr");
		DC:CallBack("ADynamicTopicDetaiHeader.SetChangeImg",index);				--	设置话题图片
		DC:CallBack("ADynamicTopicDetaiListTopic2.OnRequestTopicList",index);	--	请求话题滑动层数据
	end
end

function ADynamicTopicDetaiListTopic2:onTouchBegan(x,y)
	echo("动态话题信息点击开始");
	return true;
end

function ADynamicTopicDetaiListTopic2:onTouchMoved(x,y)
	echo("动态话题信息点击移动");
	return true;
end


function ADynamicTopicDetaiListTopic2:onTouchEnded(x,y)
	echo("动态话题信息点击结束");
	return true;
end


function ADynamicTopicDetaiListTopic2:onGuiToucBackCall(id)

end

--	请求话题列表
function ADynamicTopicDetaiListTopic2:OnRequestTopicList(str)
	self.m_topic = str;
	-- 发包
	gSink:Post("michat/get-dynamiclist",{page = 0,topic = str},function(data)
		if data.Result then
			DC:CallBack("ADynamicTopicDetaiListTopic2.SetData",data);
		else
...
--	列表数据
function ADynamicTopicDetaiListTopic2:SetData(data)
...
function ADynamicTopicDetaiListTopic2:copy(orig)
	local copytb;
...
--设置模板类型
function ADynamicTopicDetaiListTopic2:SetTemplate(data)
	if data == nil then return;end
	self.m_nIndex = {};
...
--	实时更新点赞数据
function ADynamicTopicDetaiListTopic2:updateIndex(index)
...
-- 拉黑回调
function ADynamicTopicDetaiListTopic2:DeleteUserID(userID)
...
-- 恢复拉黑回调
function ADynamicTopicDetaiListTopic2:InsertUser(userID)
	if userID == nil then return;end
...
--黑名单匹配
function ADynamicTopicDetaiListTopic2:ClearHateList(datas)
...
-- 清理
function ADynamicTopicDetaiListTopic2:Cls()
	self.Scroll:ToTop();	
	self.Scroll:DelAll();
	self.m_data = {};
	self.m_nIndex = {};
