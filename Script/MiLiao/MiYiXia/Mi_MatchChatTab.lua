...
Mi_MatchChatTab = kd.inherit(kd.Layer);
...
function Mi_MatchChatTab:init(objData)
	self.m_data = objData;
...
function Mi_MatchChatTab:GetLabelName()
	return self.m_data.Text;
end

function Mi_MatchChatTab:initView()
...
-- 设置文本
function Mi_MatchChatTab:SetLabelData(str)
...
function Mi_MatchChatTab:GetWH()
	local wl,hl = self.m_sprLeft:GetWH();
	local wr,hr = self.m_sprRight:GetWH();
	return wl+len+wr, hr;
end

function Mi_MatchChatTab:GetId()
	return self.m_data.Id;
end

function Mi_MatchChatTab:SetLabelColor()
	-- 设置颜色
